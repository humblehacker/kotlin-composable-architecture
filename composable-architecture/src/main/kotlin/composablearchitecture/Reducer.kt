...
class Reducer<State, Action, Environment>(
    ...
    fun <GlobalState, GlobalAction, GlobalEnvironment> forEach(
        ...
        Reducer { globalState, globalAction, globalEnvironment ->
            val (index, localAction) = toLocalAction(globalAction)
...
data class StateMap<GlobalState, LocalState>(
    ...
    companion object {
        fun <S> id(): StateMap<S, S> = StateMap(toLocal = { it }, fromLocal = { ls, _ -> ls })
    }
}
...
data class ActionMap<GlobalAction, LocalAction>(
    ...
    companion object {
        fun <A> id(): ActionMap<A, A> = ActionMap(toLocal = { it }, fromLocal = { it })
    }
}
